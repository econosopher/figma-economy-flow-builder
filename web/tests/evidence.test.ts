...
describe('card evidence compatibility and review',()=>{
 ...
 it('rejects mismatched videos, invalid timestamps, scripts and orphan mappings',()=>{
  const d=fixture();const item=d.evidence!.items[0];
  for(const patch of [{videoId:'aaaaaaaaaaa'},{timestampSeconds:-1},{endSeconds:10},{url:'javascript:alert(1)'},{cardIds:['absent']}]) {
   expect(()=>validateDocument({...d,evidence:{...d.evidence,items:[{...item,...patch}]}})).toThrow();
  }
 });
 it('accepts a public HTTPS image without a browser-local attachment',()=>{
  const d=fixture();const item=d.evidence!.items[0];
  expect(()=>validateDocument({...d,evidence:{...d.evidence,items:[{...item,kind:'image',url:'https://images.example.com/shop.png',mediaId:undefined,videoId:undefined}]}})).not.toThrow();
 });
