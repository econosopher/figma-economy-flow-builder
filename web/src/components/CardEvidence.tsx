...
function EvidenceImage({
  ...
  const [brokenMediaId, setBrokenMediaId] = useState<string>();
  if (image.url && brokenMediaId !== item.mediaId)
    return (
      <img
        className={className}
        src={image.url}
        alt={alt}
        onError={() => setBrokenMediaId(item.mediaId)}
      />
    );
  return (
    <div className={`${className ?? ""} evidence-image-placeholder`}>
      ...
      <span>
        {brokenMediaId === item.mediaId
          ? "This saved image could not be displayed"
          : image.missing
